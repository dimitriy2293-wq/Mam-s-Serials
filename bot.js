...
async function confirmAndEstimateCredits(ctx) {
  try {
    ...
    let balanceLine = "";
    if (balance !== null) {
      balanceLine = `\nБаланс WaveSpeed (для картинок/голоса): $${balance.toFixed(2)}.`;
      if (balance < estimatedCost) balanceLine += `\n⚠️ Баланса WaveSpeed может не хватить на весь эпизод, бот переключит ключ, если нужно. (Само видео теперь генерируется бесплатно через Digen!)`;
    }

    await ctx.reply(
      `Эпизод: ${scenes.length} сцен, ${totalSeconds} сек видео, ${ctx.session.draft.characters.length} персонажей.\n` +
      `Примерно $${estimatedCost.toFixed(2)} на картинки WaveSpeed.${balanceLine}\n\nПодтверждаешь генерацию?`,
...
async function pollScenes(ctx, episodeId) {
  ...
  while (!isDone) {
    ...
    if (pending.length === 0) {
      ...
      await ctx.reply("Видео сгенерировано! Начинаю сборку со звуком и удаление водяного знака...");
      try {
      ...
      for (const scene of pending) {
        ...
        try {
          // ИСПОЛЬЗУЕМ DIGEN ДЛЯ ПРОВЕРКИ СТАТУСА
          const status = await checkVideoStatusDigen(scene.video_job_id);
