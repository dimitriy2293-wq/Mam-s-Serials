...
// Случайный трек из assets/music (пользователь кладёт туда свои
// роялти-фри mp3 — ролик соберётся без музыки, если папка пустая).
...
export async function assembleShort(script, { onProgress } = {}) {
  ...
  // 6. Фоновая музыка (если есть файлы в assets/music) — тихая, зацикленная под длину ролика
  const musicSource = pickBackgroundMusic();
