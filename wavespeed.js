...
export async function generateSceneReferenceImage(locationImageUrl, characterImageUrls, characterPosition) {
  const prompt =
    ...
    `reference image(s) into this background, preserving each character's face, hair, and outfit ` +
    `exactly as in their reference photo. Position: ${characterPosition}. ` +
    `Correct anatomy, natural proportions, lighting on the character matching the background's lighting, ` +
    `photorealistic, sharp focus. ` +
    `Avoid: changing the background, extra limbs, extra fingers, distorted faces, text, watermark, logo, ` +
