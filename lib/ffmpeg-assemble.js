...
export async function assembleEpisode(scenes) {
  ...
  // Отдаём и публичный URL (Telegram сам скачивает видео по ссылке для
  // replyWithVideo — быстро и не упирается в исходящий канал Render), и
  // localPath как запасной вариант — если отправка по ссылке не пройдёт,
  // bot.js загрузит файл через InputFile напрямую (см. assembleShortForTelegram).
