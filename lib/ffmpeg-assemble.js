...
export async function assembleEpisode(scenes) {
  ...
  for (const scene of scenes) {
    ...
    if (scene.voiceover_audio_url) {
      ...
      try {
        // Принудительно жмем аудио в aac, чтобы формат был одинаковым у всех кусков
        execSync(
          `"${ffmpegPath}" -y -i "${rawVideo}" -i "${localAudio}" -c:v copy -c:a aac -b:a 128k -map 0:v:0 -map 1:a:0 -shortest "${merged}"`,
