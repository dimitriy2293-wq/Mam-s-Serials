...
// Настройки блюра водяного знака (правый нижний угол)
    ...
    if (scene.voiceover_audio_url) {
      ...
      try {
        ...
        if (videoDur != null && audioDur != null && audioDur > videoDur + 0.05) {
          ...
          // Добавлен блюр. Из-за фильтров пришлось заменить -c:v copy на libx264
          execSync(
            `"${ffmpegPath}" -y -i "${rawVideo}" -i "${localAudio}" ` +
      ...
      try {
        // Если озвучки нет, просто блюрим и добавляем тишину
        execSync(
