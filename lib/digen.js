...
// Заголовки для маскировки под обычный браузер при обращении к 1secmail
const secMailHeaders = {
    'Accept': 'application/json',
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
};

// Безопасный парсинг JSON с перехватом HTML-ошибок
async function safeGetJson(res, contextName) {
    const text = await res.text();
    try {
        return JSON.parse(text);
    } catch (err) {
        throw new Error(`[${contextName}] Сервер вернул невалидный JSON (Status: ${res.status}). Начало ответа: ${text.substring(0, 150)}...`);
    }
}

// Получаем рандомный email от 1secmail
async function getTempEmail() {
    // 🔥 Добавили заголовки { headers: secMailHeaders }
    const res = await fetch('https://www.1secmail.com/api/v1/?action=getDomainList', { headers: secMailHeaders });
    const domains = await safeGetJson(res, "1secmail Domain List");
...
async function getCodeFromEmail(login, domain, maxRetries = 20) {
    for (let i = 0; i < maxRetries; i++) {
        ...
        // 🔥 Добавили заголовки { headers: secMailHeaders }
        const res = await fetch(`https://www.1secmail.com/api/v1/?action=getMessages&login=${login}&domain=${domain}`, { headers: secMailHeaders });
        const messages = await safeGetJson(res, "1secmail Get Messages");
        
        if (messages.length > 0) {
            const msgId = messages[0].id;
            // 🔥 Добавили заголовки { headers: secMailHeaders }
            const msgRes = await fetch(`https://www.1secmail.com/api/v1/?action=readMessage&login=${login}&domain=${domain}&id=${msgId}`, { headers: secMailHeaders });
            const msg = await safeGetJson(msgRes, "1secmail Read Message");
...
async function createDigenAccount() {
    ...
    const verifyData = await safeGetJson(verifyRes, "Digen Verify Code");
    ...
    const regData = await safeGetJson(regRes, "Digen Register");
...
export async function generateVideoSceneDigen(params, retry = true) {
    ...
    const data = await safeGetJson(res, "Digen Submit Job");
...
export async function checkVideoStatusDigen(jobId) {
    ...
    const data = await safeGetJson(res, "Digen Check Status");
