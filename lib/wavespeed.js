...
const RATE_LIMITS = {
  "bytedance/seedance-v1-lite-i2v-480p": { max: 2, windowMs: 60_000 },
...
export async function generateVideoScene({ referenceImageUrl, prompt, durationSec }) {
  // РАНЬШЕ: wan-2.2/i2v-720p (standard) — $0.30/сцену, и даже так лица
  // "плыли" в движении. Причина не в тарифе Wan (ultra-fast vs standard),
  // а в самом семействе моделей: у открытых Wan-моделей идентичность лица
  // в движении держится заметно хуже, чем у ByteDance Seedance — она
  // регулярно показывает лучшую консистентность персонажа и стабильность
  // камеры в сравнениях. Seedance Lite i2v 480p стоит $0.08/сцену — почти
  // в 4 раза дешевле прежнего варианта, что укладывает весь эпизод (5 сцен)
  // в бюджет ~$1 вместе с картинками и озвучкой.
  // Длительность 3 сек, как просили, у Seedance технически недоступна —
  // минимум у неё 5 секунд (диапазон 5–10), меньше API не принимает.
  // Но даже на 5 сек это в разы дешевле прежней модели, так что в $1
  // укладываемся и без урезания длины.
  const duration = 5;

  const requestId = await submit("bytedance/seedance-v1-lite-i2v-480p", {
