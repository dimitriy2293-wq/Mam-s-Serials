...
export async function generateLocationImage(locationDescription) {
  ...
  // 9:16, а не 16:9 — финальный сериал должен быть вертикальным (под Reels/Shorts/
  // Telegram-плеер на телефоне), и дальше по цепочке (composite -> i2v-видео)
  // ориентация наследуется именно от этой картинки локации.
  const requestId = await submit("google/nano-banana-2/text-to-image-fast", {
    prompt,
    aspect_ratio: "9:16",
    resolution: "2k",
...
// ВАЖНО: OmniVoice принимает СТРОГО ограниченный список инструктов (см. ошибку
// WaveSpeed "Unsupported instruct items"). Раньше сюда попадали "neutral",
// "middle-aged adult" и "natural conversational tone" — ни один из них не входит
// в разрешённый список, из-за чего озвучка падала на каждой второй сцене.
// Разрешённые значения: american/australian/british/canadian/chinese/indian/
// japanese/korean/portuguese/russian accent, child, elderly, female, male,
// high/low/moderate/very high/very low pitch, teenager, whisper, young adult.
function pickVoiceDescription(characterDescription = "") {
  ...
  // Если пол не удалось определить из описания — не подставляем невалидный
  // "neutral", просто оставляем только возраст.
