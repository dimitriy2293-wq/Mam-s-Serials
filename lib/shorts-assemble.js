...
import { transcribeWithWordTimestamps } from "./assemblyai.js";
 ...
 */
export async function assembleShort(script, { voiceoverUrl, onProgress, customVisuals = [] } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < script.segments.length; i++) {
      ...
      // Если пользователь прислал свой файл для этого сегмента — используем его, иначе
      // ищем автоматически на стоках, как раньше.
      const visual = customVisuals[i] || (await findVisualForSegment(seg.visual_query || seg.narration.slice(0, 40)));
      let clipPath;
      ...
      if (visual) {
        ...
        // Сырой скачанный исходник (видео/фото) больше не нужен, как только собран
        // сегмент-клип из него. Раньше все N исходников копились в workDir до самого
        // конца сборки — а /tmp в контейнере почти всегда tmpfs (то есть физически
        // это ОЗУ), так что с каждым сегментом память росла накопительно и падала
        // где-то на середине вместо стабильного пика на каждом сегменте.
        try {
          fs.unlinkSync(rawVisualPath);
        } catch (cleanupErr) {
          console.warn(`Не удалось удалить временный исходник сегмента ${i}:`, cleanupErr.message);
        }
      } else {
      ...
      console.log(
        `[память] после сегмента ${i + 1}/${script.segments.length}: ` +
    ...
    // Отдельные клипы сегментов уже склеены в concat_video.mp4 — держать их дальше
    // в (обычно RAM-backed) /tmp до самого конца сборки смысла нет, освобождаем сразу.
    for (const clipPath of segmentClips) {
      try {
        fs.unlinkSync(clipPath);
      } catch (cleanupErr) {
        console.warn(`Не удалось удалить временный клип сегмента:`, cleanupErr.message);
      }
    }
    ...
    let words;
    try {
      words = await transcribeWithWordTimestamps(voiceoverUrl);
    } catch (err) {
      console.warn("AssemblyAI не сработал, использую приблизительный тайминг:", err.message);
      words = makeWordTimings(script, segmentDurations);
    }
    ...
    // concat_video.mp4 — полнометражный дубль без субтитров, дальше не нужен.
    try {
      fs.unlinkSync(concatVideoPath);
    } catch (cleanupErr) {
      console.warn("Не удалось удалить concat_video.mp4:", cleanupErr.message);
    }
