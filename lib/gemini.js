...
// ---------- Gemini TTS отдаёт СЫРОЙ PCM (audio/L16;codec=pcm;rate=24000), а не
// готовый .wav файл — если просто сохранить эти байты с расширением .wav,
// получится файл без RIFF-заголовка, который ffmpeg отказывается читать
// ("Invalid data found when processing input"). Дописываем заголовок сами. ----------
function pcmToWav(pcmBuffer, { sampleRate = 24000, channels = 1, bitDepth = 16 } = {}) {
  ...
  header.write("WAVE", 8);
  header.write("fmt ", 12);
  header.writeUInt32LE(16, 16); // размер fmt-блока
...
export async function generateVoiceover(text, voiceStyle) {
  ...
  // Парсим sample rate из mimeType на случай, если Google его когда-нибудь поменяет
  // (сейчас это "audio/L16;codec=pcm;rate=24000").
