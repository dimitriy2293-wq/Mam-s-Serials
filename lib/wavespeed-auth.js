...
// Храним данные текущей сессии в оперативной памяти
...
export async function step1_start(bot, chatId) {
  ...
  try {
    ...
    await bot.api.sendMessage(chatId, 
      ...
      `Введи его на сайте. Как только тебя пустит в аккаунт, напиши мне команду /done`,
      { parse_mode: "HTML" }
    );
  } catch (e) {
    await bot.api.sendMessage(chatId, "⚠️ Письмо с кодом не пришло. Попробуй начать заново.");
  }
}
...
export async function step2_finish(bot, chatId) {
  if (!sessionStore.address) {
    return bot.api.sendMessage(chatId, "❌ Нет активной сессии. Начни сначала.");
  }
  ...
  try {
    browser = await chromium.launch({
      headless: true, // ПОЛНОСТЬЮ СКРЫТЫЙ РЕЖИМ (0 визуальных лагов)
    ...
    // 2. Проверка: не просит ли GitHub код подтверждения устройства (из-за нового IP)
