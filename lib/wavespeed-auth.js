...
import { uploadToStorage } from "./storage.js"; // <-- И это
...
const browser = await chromium.launch({ 
    ...
    // proxy: { server: 'http://ip:port', username: 'логин', password: 'пароль' } // <-- Удали "//" в начале строки и впиши данные, когда купишь прокси
  });
  ...
  try {
    ...
    const context = await browser.newContext({
      userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36', // <-- Обновленная версия Chrome (126 вместо 120)
   ...
   // 1. Идем на регистрацию (увеличили таймаут до 60 сек и ждем окончания сетевых запросов)
    ...
    try {
      // 2. Ждем поле email (таймаут 30 секунд вместо 15)
