...
// ВАЖНО: ElevenLabs недавно перевёл старые "дефолтные" голоса (Adam, Rachel и т.п.)
// в Voice Library (доступ к ним истекает 31.12.2026) — и на бесплатном тарифе они
// ТОЖЕ теперь блокируются по API той же ошибкой "paid_plan_required", что и любой
// другой библиотечный голос. То есть подставлять другой готовый voice_id вместо
// сломанного бесполезно — free-tier API вообще не даёт использовать ЧУЖИЕ голоса
// (ни библиотечные, ни бывшие дефолтные), а только свои — созданные через Instant
// Voice Cloning или Voice Design в личном кабинете ElevenLabs. Поэтому вместо
// молчаливой (и обречённой) попытки подставить другой чужой голос — сразу кидаем
// понятную ошибку с инструкцией, что делать. Подробности — в README.
function isPaidPlanRequiredError(err) {
  return err?.status === 402 || /paid_plan_required/i.test(err?.message || "");
}

function friendlyVoiceError(originalErr) {
  const err = new Error(
    "ElevenLabs: этот голос недоступен на бесплатном тарифе через API (ошибка 402 " +
      "paid_plan_required) — так теперь происходит с ЛЮБЫМ чужим голосом (и Voice " +
      "Library, и бывшими дефолтными вроде Adam/Rachel), не только с тем, что ты " +
      "выбрал. Два варианта: 1) создать СВОЙ голос (Instant Voice Cloning или Voice " +
      "Design в личном кабинете ElevenLabs → Voices → Add voice) и указать его ID " +
      "в .env, 2) перейти на платный тариф ElevenLabs (Starter и выше) — тогда любые " +
      "голоса работают через API без ограничений."
  );
  err.status = originalErr?.status;
  return err;
}
...
export async function generateVoiceover(text, characterName, characterDescription) {
  ...
  try {
    ...
    if (isPaidPlanRequiredError(err)) throw friendlyVoiceError(err);
    throw err;
  }
