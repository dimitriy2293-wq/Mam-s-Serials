...
async function generateVoiceoverViaBrowser(text) {
  ...
  try {
    ...
    // ДОБАВЛЕННЫЙ БЛОК: Ожидание поля email с таймаутом и скриншотом при ошибке
   try {
  // Твой существующий код перехода на сайт
  await page.goto('https://elevenlabs.io/sign-in', { waitUntil: 'networkidle' });

  // Ждем поле ввода email
  await page.waitForSelector('input[type="email"]', { timeout: 30000 });

  // ... дальше твой стандартный код ввода лога/пароля ...

} catch (error) {
  console.error('Ошибка при входе в ElevenLabs:', error);

  // 1. Делаем скриншот экрана браузера
  const screenshotBuffer = await page.screenshot({ fullPage: true });

  // 2. Отправляем скриншот в Telegram
  // Замени chatId на переменную с ID твоего чата
  await bot.sendPhoto(chatId, screenshotBuffer, {
    caption: `⚠️ **Ошибка Playwright:**\n\`${error.message}\`\n\nВот что сейчас видит браузер:`
  });

  // Пробрасываем ошибку дальше, чтобы скрипт корректно завершил попытку
  throw error;
}
